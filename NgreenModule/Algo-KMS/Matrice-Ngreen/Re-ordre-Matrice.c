...
/*-------------- Generation de la Matrice A et la Matrice Epsilon1 -------------*/	 
	 ...
	 for(i=0;i<c-c1;i++)
	 {
				...
				fprintf(fA3,"%12d %12d ",i,NbEvtsPossibles-1);
				fprintf(fAps3,"%12d %12d",i,bool);
					  for(j=0;j<degre;j++)
					  {
							fscanf(fRiiOrd,"%Le %d",&proba,&e);
							if(e<c)
							fprintf(fA3,"% .15Lf%12d",proba,e); 
							else
							fprintf(fAps3,"% .15Lf%12d",proba,e);
					  }
					  fprintf(fA3,"\n");	
					  fprintf(fAps3,"\n");	
	 }
	...
	for(i=c;i<c+(c-c1);i++) 
	 {
				...
				fprintf(fB3,"%12d %12d",i-c,NbEvtsPossibles-1);
				
					  for(j=0;j<degre;j++)
					  {
							fscanf(fRiiOrd,"%Le %d",&proba,&e);
							if(e<c)
							fprintf(fBps3,"% .15Lf%12d",proba,e); 
							else
							fprintf(fB3,"% .15Lf%12d",proba,e-c);
					  }
					  fprintf(fB3,"\n");	
					  fprintf(fBps3,"\n");	
	 }
