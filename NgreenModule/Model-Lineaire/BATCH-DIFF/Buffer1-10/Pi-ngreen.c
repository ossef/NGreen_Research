...
int somme(int t[], int n, int h)
{
int i;
int s =0;
if(n<h) {printf("Erreur l'etat (X,%d,M) n'existe pas \n",h);exit(2); }
...
char *argv[]; 
{ 
	int NArcs,size,k,i,j,d,e,c,r1,r2;
	long double prb,test,som;
...
/* Extraction d'information du fichier filename.cd*/
	fcd=fopen(s2,"r");
	fseek ( fcd , -9*5 , SEEK_END ); //Lecture du fichier à partir de la fin
        ...
        if((i>2) && (x<=seuil) && ( h != Deadline) )
		{
			  ...
			  if(m == 0)
			  {
			   if( d>1)
				 ...
				 for (j = 0; j < d; j++) 
				  {
					...
					if(e<i)
						{
							  //printf("h = 1 , d>1 : etats %d : (%d,%d,%d) vers %d avec proba %Lf \n",i,x,h,m,e,prb);
				...
				if( d == 1)
				{
					fscanf(fcii,"%Lf",&prb);
					fscanf(fcii,"%d",&e);
					//printf("h = 1, d = 1 : etats %d : (%d,%d,%d) vers %d avec proba %Lf \n",i,x,h,m,1+recherche1(Tab,Deadline+1,i),Tprob2[r1]);
					pi[i] += ((alpha10*Tprob2[r1++]*pi[1+recherche1(Tab,Deadline+1,i)])*conste);
				}
			   }
			...
			else //if(h != 1)
			{
			  if(m ==0)
			  {
					...
					for (j = 0; j < d; j++) 
							{
								...
								if (j == 0 )
								{
								 //printf("h != 1 , etats %d : (%d,%d,%d) vers %d avec proba %Lf \n",i,x,h,m,e,prb);
								 pi[i] += ((prb*pi[e])*conste);
								}
							}
				}
				...
				{
					if( i == (somme(Tab,Deadline+1,h) + Tab[h] -1) )
					{	
						if( d != 1) erreur(i,s3);
						fscanf(fcii,"%Lf",&prb);
						fscanf(fcii,"%d",&e);
						//printf("h != 1 , etats %d : (%d,%d,%d) vers %d avec proba %Lf \n",i,x,h,m,e-2*Tab[h],Tprob2[Tbatch-1]);
						pi[i] += ((alpha10*Tprob2[Tbatch-1]*pi[e-2*Tab[h]])*conste);
					}
					else
					{
					/*	
						for (j = 0; j < d; j++) 
						  {
							...
							if(e<i)
								{
									  pi[i] += ((prb*pi[e])/(1-alpha01*alpha10));
									  pi[i] += ((alpha10*recherche2(Tprob1,Tprob2,Tbatch,prb)*pi[e-1+recherche1(Tab,Deadline+1,i)])/(1-alpha01*alpha10));
								}
						  }
					  */
					  if( (i%2) == 0)
						...
						for (j = 0; j < d; j++) 
						  {
							...
							if(e<i)
								{
									  pi[i] += ((prb*pi[e])*conste);
									  //printf("h != 1, pair , etats %d : (%d,%d,%d) vers %d avec proba %Lf et vers %d avec %Lf  \n",i,x,h,m,e,prb,e-1+recherche1(Tab,Deadline+1,i),recherche2(Tprob1,Tprob2,Tbatch,prb));
									  pi[i] += ((alpha10*recherche2(Tprob1,Tprob2,Tbatch,prb)*pi[e-1+recherche1(Tab,Deadline+1,i)])*conste);
								}
						  }
					   }
					   else
								...
								if(e<i)
									{
										if(j == 0)
										{
											 //printf("h != 1, impair , etats %d : (%d,%d,%d) vers %d avec proba %Lf \n ",i,x,h,m,e,prb);
											pi[i] += ((prb*pi[e])*conste);	
										}
										else
										{
												  //printf("puis vers %d avec proba %Lf et %d avec proba %Lf \n",e,prb,e-1+recherche1(Tab,Deadline+1,i),recherche2(Tprob1,Tprob2,Tbatch,prb));
												  pi[i] += ((prb*pi[e])*conste);
												  pi[i] += ((alpha10*recherche2(Tprob1,Tprob2,Tbatch,prb)*pi[e-1+recherche1(Tab,Deadline+1,i)])*conste);
										}
									}
		  ...
		  }// fin if( not seuil et not deadline)
