...
long double * Puissance(long double *pi, int size)
{
	int i, j;
	long double som;
	long double *pin;
...
/* 5 - 2 Effectuer une iteration de l'agorithme de puissance Pi*M , en prenant pi et piZ en entrée */
#if (Residu == 2)
	piAvnt = copie(pi,size);
#endif


pi = Puissance(piZ,size); //pi = Gauss_Seidel_Iter(pi,piZ,size,tabD,tabF,parts,tabFond,tfond);
...
/*-----------Fin de l'execution du programme---------------------*/
 ...
 ftime = fopen("KMS_OPT_POW.time","a");
 fprintf(ftime,"%.2lf\n",(((tv2.tv_sec-tv1.tv_sec)*1000000L +tv2.tv_usec) - tv1.tv_usec)*0.000001);
 fclose(ftime);
 printf("ALGO KMS BASIC OPTIMALE (POW) DONE, temps: %.2lf secondes\n", (((tv2.tv_sec-tv1.tv_sec)*1000000L +tv2.tv_usec) - tv1.tv_usec)*0.000001); 
