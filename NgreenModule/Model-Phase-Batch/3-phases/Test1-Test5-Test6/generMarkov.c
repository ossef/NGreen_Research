...
static int Min[NEt], Max[NEt], Deadline;
static int seuil;
...
void  usage()
{
	printf("usage : GenerMatrix -f filename seuil deadline \n");
...
char *argv[];
{
    ...
    for (j = 1; j <= bcli; j++) {
		 ...
		 */
		fprintf(pf2,"% .15Le%12d", It2.prob, It2.numero);
	...
	{
		printf("attention la somme vaut : %.15Le\n", SommeDeProba);
		exit(0);
	}
