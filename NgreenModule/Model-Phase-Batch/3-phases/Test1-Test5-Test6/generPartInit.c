...
/* On ecrit l'approximation 1/N dans le fichier "filenale.pi0" */
	...
	if(size %PHASES == 1)
	{
		printf("Probleme de taille, la tailles de votre matrice doit être divisble par 3 (Matrice 3 block)\n");
		exit(0);
	}
	fpart=fopen(s2,"w");
	fprintf(fpart," 3 \n");
	fprintf(fpart," 0 %d  ", size/PHASES);
	for (i = 0; i < size/PHASES; i++) {
		fprintf(fpart," %d ", i);
	}
	fprintf(fpart,"\n");
	fprintf(fpart," 1 %d  ",size/PHASES);
	for (i = size/PHASES; i < 2 * (size/PHASES); i++) {
		fprintf(fpart," %d ", i);
	}
	fprintf(fpart,"\n");
	fprintf(fpart," 2 %d  ",size/PHASES);
	for (i =  2 * (size/PHASES); i < size; i++) {
		fprintf(fpart," %d ", i);
	}
	fclose(fpart);
