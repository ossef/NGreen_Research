/*-- Version finale du gth creux
 *-- fait le 10/03/96
 *-- Le fichier en entree est de la forme suivante
 *--      * La premiere valeur correspond au nombre  de valeurs non nulles dans la matrice
 *--	* la deuxieme valeur correspond a la taille de la matrice
 *-- 	* Ensuite, il faut donner les elements non nuls de la matrice ligne par ligne,  chaque ligne contient :
 *--		-- le nombre d'elements non nuls
 *-- 		-- Les elements un par un, en donnant d'abord la valeur suivie du numero de colonne.
*/
...
void  Construire_Matrice_Transition(char fich[200]) 
{
	...
	if ((G=fopen(s1,"r"))==NULL)
        {
          puts("Le Fichier specifie n'existant pas") ;
          exit(1) ;
	}  
...
if(argc !=2 )
 {
  ...
  exit(1);
	  ...
	  //printf("%s",);
	  printf("\n j'essaye de passer \n !");
      Construire_Matrice_Transition(*(++argv));
      printf("%s !",*argv);
