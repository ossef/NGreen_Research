...
void Marg_Cond_X_H(int tab[],double pi[],int taille,double seuil,int Deadline,char s5[50],char s6[50]) // Marginal conditionnée sur X (en sortie) puis sur H
{
long double som,s1,s2;
...
for(i=0; i<(seuil*Buffer)+a[NbEvtsPossibles-1]+1; i=i+1){ tab1[i] = 0;}
...
f1=fopen(s5,"w");
	for(i=0; i<(seuil*Buffer)+a[NbEvtsPossibles-1]+1; i=i+1){ 
#if (DEBUG==1)	
	printf("tab1[%d] =      %.18Le\n",i,tab1[i]);
#endif
	tab1[i] /= som;
...
f2=fopen(s6,"w");
	...
	printf("Moyenne d'envoie d'un conteneur : %.18Le\n",moyenne);
