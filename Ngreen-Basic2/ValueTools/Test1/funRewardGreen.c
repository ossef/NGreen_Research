...
{
long double som,s1,s2,test,test1,test2,test3,test4,test5,test6,test7;
long double *tab1 = calloc((int)(seuil*Buffer)+a[NbEvtsPossibles-1] + 1,sizeof(long double)); // des X
long double *tab2 = calloc(Deadline+1,sizeof(long double));   // des H
...
fclose(f2);
	...
	printf("La moyenne le la loi X de remplissage est : %Lf \n",som);
	...
	test1 = 0;
	for(i=0; i<(int)(seuil*Buffer)+a[NbEvtsPossibles-1] + 1 ; i=i+1){ 
		if(i<(int)(seuil*Buffer))
			test1 += tab1[i]; 
	}
	
	test2 = tab2[Deadline];
	
	printf(" \n somme1(avant pique) =  %Lf et s1 = %Lf et s2 = %Lf et s3 = %Lf et s5 = %Lf \n",test1,test6,test7,test5,test2);
