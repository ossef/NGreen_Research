...
/*Affichage X pour chaque experience*/
	som =0;
	for(i=0; i<(int)(seuil*Buffer)+a[NbEvtsPossibles-1] + 1 ; i=i+1){ 
	som += i*tab1[i];
	}
	...
	f3 = fopen("ResultatsX.data","a");
	f5 = fopen("ResultatsE.data","a");
	fprintf(f3,"%5d  %Lf",Deadline,som); // affiche remplissage
	fprintf(f5,"%5d  % .15LE",Deadline,((80*(1e-6))/(som*50*8))*1e9); //affiche efficacité d'énergie
	if(seuil*Buffer >= 216)
		{fprintf(f3,"\n");fprintf(f5,"\n");}
	fclose(f3);
	fclose(f5);

/*Affichage H pour chaque experience*/	
    som = 0;
	for(i=0; i<Deadline+1; i=i+1){
		som += i*tab2[i];	
	}
	...
	f4 = fopen("ResultatsH.data","a");
	fprintf(f4,"%5d  %Lf",Deadline,som);
	if(seuil*Buffer >= 216)
		fprintf(f4,"\n");
	fclose(f4);
...
void Convolution_H_G(int tab[],double pi[],int taille,double seuil,int Deadline,char s5[50],char s6[50],char s7[100]) /*------- Convolution du timer H et de G qui suit une loi geomtrique de (1-p(a[0])) -------*/
{ 
 long double s = 0;
 long double *tab2 = calloc(Deadline+1,sizeof(long double));
