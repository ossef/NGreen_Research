...
{ 
 ...
 return k<0 ? 0 : pow(param,k)*(1-param);	
}
...
{ 
 long double *tab2 = calloc(Deadline+1,sizeof(long double));
 long double *tabConv = calloc(Deadline+1,sizeof(long double));
 ...
 if(i<NbEvtsPossibles) // on a bien une arrivée avec 0 SDU ==> Calcule de la convolution
	{
	   ...
	   double param = p0;
	   for(x=0; x<Deadline+1; x++)
			{
				for(int h=0; h<Deadline+1; h++)
				{
					tabConv[x] += tab2[h]*Geometrique(param,x-h);	
					//printf("TabH[%d] = %Lf et Geomtrique (%d) = %f \n",h,tab2[h],x-h,Geometrique(param,x-h));
				} 
			} 
		
		FILE *f1=fopen(s7,"w");
		long double som =0;
		for(x=0; x<Deadline+1; x++){ som += tabConv[x];} 
		for(x=0; x<Deadline+1; x++)
				{
					tabConv[x] /= som;
#if (DEBUG==1)	
