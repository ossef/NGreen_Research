...
{ 
    ...
    /* Calcul du nombre moyen de client en attente dans la file  */
     ...
     /* Calcul du temps d'attente ou temps d'insertion  */
     
       TempsInsertion(et,pi,ReachableStates,s4);
    ...
    pf1=fopen(s5,"a");
    fprintf(pf1,"Periode : %5d    ", D); //%.20lf
    fprintf(pf1,"Pertes : % .15E    ", perte); //%.20lf
    //fprintf(pf1,"%.12lf  ", Consenergie);
    fprintf(pf1,"NbreMoyen : % .15E   ", NombreMoyn);
    fprintf(pf1,"Attente : % .15E    ", TimeAttente);
