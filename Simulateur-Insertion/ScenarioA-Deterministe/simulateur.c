...
#define TAILLEMAX 100 //taille de l'anneau
...
int IndiceAn[TAILLEMAX]; // l'anneau est remplit par des slots contenant (-1 pour vide  ou l'indice de l'emetteur)
...
void decalage_Anneau()
{
	...
	int tmp2 = IndiceAn[TAILLEMAX- 1];
	memmove(&IndiceAn[1], IndiceAn, (TAILLEMAX - 1) * sizeof(int));
	IndiceAn[0] = tmp2;
}
...
void Generer_Fonct_Repart()
{
    ...
    FILE *f1 = fopen("test-ngreen-mod2-250.Conv.H.G.pi","r"); //("test-ngreen-25-DAG.Conv.H.G.pi","r"); // Je lis la distribution des arrivées.
	 ...
	 for(x1=0; x1<taille; x1++)
			{
				...
				fprintf(final,"%5d %.18Le \n",x1+TAILLEMAX,TabConv3[x1]); // on raoute la taille de l'anneau
			}
			fclose(final);
...
t = 0;
 do 
 {
	...
	fprintf(f1,"%10d",Compteurs[i]); // affichage #waiting PDUs 
		...
		{
			if(Anneau[PlaceStation[i]] == -1 && DeltaStation[i] == 0 && NbrePDUs[i]>0 && DateArrivee[i][IndiceEvent[i]] != -1 && (IndiceAn[i]%10) == i) //Je reçoit un Slot vide et delai d'emission atteint, et file non vide 
