...
for (i in ma_seq){ #pour chaque Station son propre plot
plot(data2[,i],data2[,i+1],type="s",col="black",xlab="Arrival date of PDUs", ylab="Insertion Delay", xlim=c(0,10000), ylim=c(0,35))#,xlim=c(0,m1), ylim=c(0,m2))
tab = table(data2[,i+1])
tab = data.frame(tab) 
som = sum(as.vector(tab[,2]))
barplot(as.vector(tab[,2])/som, names.arg=as.vector(tab[,1]),xlab = "Insertion Delay",ylim = c(0,0.45),ylab = "Probability",col = "blue", border = "black")
}
...
plot(V1,V2,type="l",ylim=c(0,120), col="black",xlab="time", ylab="Filled slots")
