...
#define TIMEMAX 100000   //temps max de simulation ?
#define TAILLEMAX 150 //taille de l'anneau
#define STATIONS 29 //nombre de stations
#define DEADLINE 108 //Deadline tronquée liée à la distribution de H conv G  (voir modèle Buffer25)
#define BUFFER 4500 // taille du conteneur de PDUs (taille echeancier)
...
int taille2 = 600;
long double *TabConv2 =  calloc(taille2, sizeof(long double));
int taille = DEADLINE+1+taille2; //tronquature
...
fclose(f1);	
	...
	for ( i=0; i<BUFFER ; i++) {
	 ...
	 printf("la moyenne de Conv1 : %Lf \n",t);

	 
	 for ( i=0; i<taille2 ; i++) { //Geometrique tronquée à taille2
		 s += TabConv2[i];
	 }
	 for ( i=0; i<taille2 ; i++) {
	 ...
	 for ( i=0; i<taille2 ; i++) {
	  ...
	  printf("la moyenne de Conv2 : %Lf \n",t);
	  FILE *fint = fopen("STATIONS29.InsrtTime","a");
	  fprintf(fint,"%.2Lf\n",t);
	  fclose(fint);
