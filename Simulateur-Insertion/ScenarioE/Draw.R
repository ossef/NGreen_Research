...
for (i in ma_seq){ #pour chaque Station son propre plot
plot(data2[,i],data2[,i+1],type="s",col="black",xlab="Arrival date of PDUs", ylab="Delay", xlim=c(0,10000), ylim=c(0,40))#,xlim=c(0,m1), ylim=c(0,m2))
tab = table(data2[,i+1])
tab = data.frame(tab) 
som = sum(as.vector(tab[,2]))
barplot(as.vector(tab[,2])/som, names.arg=as.vector(tab[,1]),xlab = "Delay",ylim = c(0,0.3),ylab = "Probability",col = "blue", border = "black")
}
